...
# instantiate digit recognizer once to avoid reloading model every frame
...
while(True):
    ...
    # preprocess and predict with the already-instantiated drmObj
    model_img = drmObj.imgPreprocessing(imgBW = imgBW)
    try:
        digit, conf = drmObj.predict(model_img)
        # display prediction on the frame
        cv2.putText(frame, f"Pred: {digit} ({conf:.2f})", (240,70), cv2.FONT_HERSHEY_PLAIN, 3, (0,255,0), 3)
    except Exception as e:
        # if prediction fails, show a warning
        cv2.putText(frame, "Pred: N/A", (240,70), cv2.FONT_HERSHEY_PLAIN, 3, (0,0,255), 3)
